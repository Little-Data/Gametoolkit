...
## <span style={{color: '#FF00FF'}}>R3000A指令集</span>
...
## <span style={{color: '#FF00FF'}}>指令集</span>
下面的表是R3000A的汇编指令。

## <span style={{color: '#FF00FF'}}>装入和储存指令</span>
...
## <span style={{color: '#FF00FF'}}>算术指令</span>
...
## <span style={{color: '#FF00FF'}}>跳转和转移指令</span>
...
| Jump and Link | JAL  target<br/>左移26位target地址，加上PC的高4位，1个指令延时后跳转。下一个指令地址放到r31（链接寄存器） |
...
## <span style={{color: '#FF00FF'}}>特殊指令</span>
...
## <span style={{color: '#FF00FF'}}>协处理器指令</span>
...
## <span style={{color: '#FF00FF'}}>R3000A操作码编码</span>
下面是MIPS结构的操作码编码
